...
datasets = ['./data/offline-train.pkl', r'./data/train-caption.txt']
valid_datasets = ['./data/offline-test-2019.pkl', './data/test-caption-2019.txt']
...
for eidx in range(max_epochs):
    ...
    for x, y in train:
        ...
        if np.mod(uidx, dispFreq) == 0:
            ...
            loss_s2 /= dispFreq
            loss_kl /= dispFreq
            print('Data', datetime.now().strftime('%Y-%m-%d %H-%M-%S'), 'Epoch', eidx, 'Update/total: ',
                  str(uidx) + '/' + str(len(train)), 'Cost_L2R: ', loss_s, 'Cost_R2L: ', loss_s2, 'KL: ', loss_kl,
                  'time ', ud_s, 'lrate ', lrate, 'eps', my_eps, 'bad_counter', bad_counter)
            ud_s = 0
            loss_s = 0.
            loss_s2 = 0
            loss_kl = 0
        ...
        if True:
            if np.mod(uidx, validFreq) == 0:
                ...
                with torch.no_grad():
                    for x, y in valid:
                        x, x_mask, y, y_out, y_mask, y_reverse, y_reverse_out, y_reverse_mask = prepare_data_bidecoder(
                            params, x, y)
                        x = torch.from_numpy(x)  # .cuda()
                        x_mask = torch.from_numpy(x_mask)  # .cuda()
                        ...
                        scores, alphas, scores2, alphas2 = model(params, x, x_mask, y, y_mask, y_reverse,
                                                                 y_reverse_mask)
            ...
            with torch.no_grad():
                ...
                for x, y in valid:
                    for xx in x:
                        ...
                        sample, score, _, _ = gen_sample_bidirection(model, xx_pad, params, multi_gpu_flag, k=10,
                                                                     maxlen=1000)
                        if len(score) == 0:
        ...
        if np.mod(uidx, validFreq) == 0 and valid_stop == False:
            ...
            if valid_sacc > max_ExpRate:
                ...
                if multi_gpu_flag:
                    torch.save(model.module.state_dict(),
                               save_model_path + '/ABM_params' + str(eidx) + '_lr' + str(lrate) + '_' + str(
                                   valid_sacc) + '.pkl')
                else:
            ...
            print('Valid WER: %.2f%%, ExpRate: %.2f%%' % (valid_err, valid_sacc),
                  'min Valid WER: %.2f%%, max ExpRate: %.2f%%' % (min_Valid_WER, max_ExpRate))
