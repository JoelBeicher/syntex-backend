...
def gen_sample_bidirection(model, x, params, gpu_flag, k=1, maxlen=30, idx_decoder=1):
    ...
    for ii in range(maxlen):
        ...
        if gpu_flag:
            next_p, next_state, next_alpha_past, alpha = model.module.f_next(params, next_w, None, ctx, None,
                                                                             next_state,
        ...
        next_p = next_p.detach().numpy()
        next_state = next_state.detach().numpy()
        next_alpha_past = next_alpha_past.detach().numpy()
