...
def extract_data(archive: ZipFile, dir_name: str, ext: str) -> Data:
    ...
    for line in captions:
        ...
        img_name = tmp[0]
        formula = tmp[1:]
        with archive.open(f"{dir_name}/{img_name}{ext}", "r") as f:
...
def build_dataset(archive, folder: str, ext: str, batch_size: int):
    data = extract_data(archive, folder, ext)
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        with ZipFile(self.zipfile_path) as archive:
            if stage == "fit" or stage is None:
                self.train_dataset = build_dataset(archive, "train", self.image_ext, self.batch_size)
                self.val_dataset = build_dataset(archive, self.test_year,  self.image_ext, 1)
            if stage == "test" or stage is None:
                self.test_dataset = build_dataset(archive, self.test_year, self.image_ext, 1)
